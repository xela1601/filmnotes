...
If you want to narrow it down, uncomment the `command:` block in `compose.yaml` and add the
...
`compose.yaml` does **not** mount such a directory, so if you want that, add a bind mount
