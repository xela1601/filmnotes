...
- [x] **`README.md`** – what it is (2 paragraphs), screenshots placeholder section (list which screens to capture), monorepo layout (from spec §3), quick start (`npm install`, `npm test`, `npm run web -w @filmnotes/mobile`, backend `fetch-pb` + `start:dev`), building for devices (`npx expo run:ios` / `run:android`, EAS notes), licence MIT, link to `docs/`.
- [x] **`docs/workflow.md`** – the routine, as numbered checklists with the exact screens/buttons:
