...
| 4   | **GNU make** and **jq**                                                                                             | the targets in `sandbox/Makefile`, token extraction                                                                | `make --version`, `jq --version`                                                                                  |
| 5   | **Bitwarden CLI** (`bw`), logged in, with the item **"My Vault" > GitHub > "sbx-filmnotes"** holding a GitHub token | the `github` secret of the sandbox: read on the host at creation, injected by the proxy, only a placeholder inside | `bw status` shows `locked` or `unlocked` (not `unauthenticated`); `make sbx-create` unlocks the vault when needed |
| 6   | Network access to **Docker Hub** and **GitHub**                                                                     | base image, PocketBase binary                                                                                      | only while building                                                                                               |

Nothing else: no Node on the host, no secrets in any file. The environment file names no user;
...
make -C sandbox sbx-create       # unlocks Bitwarden if needed, reads the GitHub token
