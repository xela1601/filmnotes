...
const fakeContents = (FileSystem as unknown as { __contents: Map<string, Uint8Array> })
  .__contents;
...
describe('isZip', () => {
  it('recognises a ZIP by name and by mime type', () => {
    expect(isZip({ name: 'roll.ZIP', uri: 'x', mimeType: 'application/octet-stream', size: 1 })).toBe(
      true,
    );
    expect(
