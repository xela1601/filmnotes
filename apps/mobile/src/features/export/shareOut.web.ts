...
export async function shareOut(payload: SharePayload): Promise<void> {
  const files =
    payload.image === null
      ? []
      : [new File([blobOf(payload.image)], payload.image.fileName, { type: payload.image.mimeType })];

  const data: ShareData = payload.image === null ? { text: payload.text } : { text: payload.text, files };
