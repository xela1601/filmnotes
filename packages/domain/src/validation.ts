...
import { compareShutterSpeeds, isBulb, isBulbAllowedInMode, isSlowerThan, shutterSpeedsForMode } from './shutter';
...
const filterThread: Rule = (frame, { lens, filters }) => {
  ...
  return filters
    ...
    .map((filter) => issue('error', 'filter_thread_mismatch', 'filterIds', {
      filter: filter.model,
...
const frameNoUnique: Rule = (frame, { siblingFrames }) => {
  const duplicate = siblingFrames.some(
    (sibling) => sibling.id !== frame.id && sibling.deleted === null && sibling.frameNo === frame.frameNo,
  );
