...
function frame(frameNo: number, overrides: Partial<Frame> = {}): Frame {
  return makeFrame({ id: `frame${String(frameNo).padStart(2, '0')}0000000`, frameNo, ...overrides });
}
...
describe('matchScansToFrames', () => {
  ...
  it('leaves surplus scans unassigned', () => {
    ...
    expect(assignments[3]).toEqual({ fileName: 'a4.jpg', sortIndex: 3, frameId: null, frameNo: null });
  });
...
describe('shiftAssignments', () => {
  ...
  it('moves every assignment from the given index to the next frame', () => {
    ...
    expect(shifted.map((assignment) => assignment.fileName)).toEqual(['a1.jpg', 'a2.jpg', 'a3.jpg']);
  });
