...
describe('wordPressExporter', () => {
  ...
  it('accepts a complete config and rejects an invalid one', () => {
    ...
    expect(() => wordPressExporter.configSchema.parse({ ...CONFIG, siteUrl: 'not a url' })).toThrow();
  ...
  it('skips the media upload and the featured image when there is no image', async () => {
    const { deps, requests } = fakeFetch([{ json: { id: 101, link: 'https://blog.example/?p=101' } }]);
    const input = makeInput({ image: null });

    const result = await wordPressExporter.exportFrame(input, { ...CONFIG, status: 'publish' }, deps);
  ...
  it('throws a WordPressError when the post cannot be created', async () => {
    ...
    ).rejects.toMatchObject({ name: 'WordPressError', status: 401, message: expect.stringContaining('Unauthorized') });
  });
});
...
describe('buildPostHtml', () => {
  ...
  it('leaves out rows whose value is unknown', () => {
    ...
    for (const label of ['Lens', 'Focal length', 'Aperture', 'Shutter', 'Mode', 'Filters', 'Location', 'Date']) {
  ...
  it('has no notes paragraph when the frame has no notes', () => {
    expect(buildPostHtml(makeInput({ frame: makeFrame({ notes: '  ' }) }), null)).not.toContain('<p>');
  });
