...
function parsed<T>(result: z.SafeParseReturnType<unknown, T>): T {
  if (!result.success) throw new Error(`data does not match its schema:\n${describeIssues(result.error)}`);
  return result.data;
}
  ...
  it('only references ids that exist in the bundle', () => {
    ...
    for (const lens of bundle.lenses) {
      for (const filterId of lens.defaultFilterIds) {
        expect({ lens: lens.id, filterId, known: filterIds.has(filterId) }).toMatchObject({ known: true });
      }
    }
    for (const filter of bundle.filters) {
      if (filter.mountedOnLensId === null) continue;
      expect({ filter: filter.id, lensId: filter.mountedOnLensId, known: lensIds.has(filter.mountedOnLensId) })
    ...
    for (const camera of bundle.cameras) {
      ...
      if (defaults.lensId !== null) {
        expect({ camera: camera.id, lensId: defaults.lensId, known: lensIds.has(defaults.lensId) })
          .toMatchObject({ known: true });
      }
      for (const filterId of defaults.filterIds) {
        expect({ camera: camera.id, filterId, known: filterIds.has(filterId) }).toMatchObject({ known: true });
      }
      if (defaults.flashId !== null) {
        const flashIds = new Set(bundle.flashes.map((flash) => flash.id));
        expect({ camera: camera.id, flashId: defaults.flashId, known: flashIds.has(defaults.flashId) })
          .toMatchObject({ known: true });
      }
    }
  });
